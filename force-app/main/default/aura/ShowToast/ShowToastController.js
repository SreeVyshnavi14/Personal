({
    invoke : function(component, event, helper) {
        ...
        var type = component.get("v.type") //force user entered attribute to all lowercase
        var title = component.get("v.title");
        var message = component.get("v.message");
        var duration = component.get("v.duration")+"000"; //convert duration value from seconds to milliseconds
        var mode = component.get("v.mode") //force user entered attribute to all lowercase
        var key = component.get("v.key")   //force user entered attribute to all lowercase
        
        var isURL;
        if(message){
            isURL = message.includes('{url}');  //Did the user include '{url}' in their message?
        }
